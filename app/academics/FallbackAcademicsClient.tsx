...
/* ─── Page ─── */
export default function FallbackAcademicsClient() {
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Link href="/academics/basic-education">
            <motion.div
              ...
              transition={{ duration: 0.6 }}
            >
              <Image src="/images/homepage2.jpg" alt="Basic Education" fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-7">
                ...
                <p className="text-gray-300 text-sm leading-relaxed mb-4 max-w-xs">Grade School through Senior High School — foundational learning rooted in Savian values.</p>
                <span className="inline-flex items-center gap-2 text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Explore <ArrowRight className="w-4 h-4" /></span>
              </div>
            </motion.div>
          </Link>
          ...
          <Link href="/academics/college-programs">
            ...
            >
              <Image src="/images/homapage3.png" alt="College Programs" fill className="object-cover object-top transition-transform duration-700 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-7">
                ...
                <p className="text-gray-300 text-sm leading-relaxed mb-4 max-w-xs">Undergraduate and professional degree programs across five academic departments.</p>
                <span className="inline-flex items-center gap-2 text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">Explore <ArrowRight className="w-4 h-4" /></span>
              </div>
            </motion.div>
          </Link>
      ...
      <section className="bg-gray-50 py-20">
        <div className="max-w-6xl mx-auto px-4">
          <motion.div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <div>
              ...
              <p className="text-gray-500 text-sm md:text-base max-w-lg">Five departments offering career-focused undergraduate and professional degrees.</p>
            </div>
            <Link href="/academics/college-programs" className="inline-flex items-center gap-2 text-green-700 text-sm font-semibold hover:gap-3 transition-all shrink-0">View all programs <ArrowRight className="w-4 h-4" /></Link>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {collegeDepartments.map((dept, idx) => (
              <motion.div key={idx} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border-l-4 border-l-green-500" custom={idx} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
                ...
                <ul className="space-y-2">
                  {dept.courses.map((course, cIdx) => (
                    <li key={cIdx} className="flex items-start gap-2 text-sm text-gray-600">
                      <span className="mt-[7px] w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" />{course}
                    </li>
                  ))}
                </ul>
              </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <motion.div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <div>
              ...
              <div className="w-12 h-0.5 bg-green-600 mt-2 mb-3 rounded-full" />
              <p className="text-gray-500 text-sm md:text-base max-w-lg">Quality K-12 programs building strong foundations rooted in Savian values.</p>
            </div>
            <Link href="/academics/basic-education" className="inline-flex items-center gap-2 text-green-700 text-sm font-semibold hover:gap-3 transition-all shrink-0">View all programs <ArrowRight className="w-4 h-4" /></Link>
          </motion.div>
